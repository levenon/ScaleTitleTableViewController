...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated nearestPosition:(MMHorizontalListViewPosition)position {
    ...
    if (cellVisibleFrame.origin.x < -self.contentInset.right) {
        cellVisibleFrame.origin.x = -self.contentInset.right;
    } else if (cellVisibleFrame.origin.x > self.contentSize.width + self.contentInset.right - self.frame.size.width ) {
...
- (void)selectCellAtIndex:(NSInteger)index animated:(BOOL)animated {
    if (!_allowsMultipleSelection) {
        [self _deselectCellExcludeIndex:index animated:animated responseToDelegate:YES];
    }
    if (_allowsSelection) {
        [self _selectCellAtIndex:index animated:animated responseToDelegate:NO];
    }
}

- (void)_selectCellAtIndex:(NSInteger)index animated:(BOOL)animated responseToDelegate:(BOOL)responseToDelegate{
    ...
    if (cell && ![cell selected]) {
        ...
        if (responseToDelegate && [_horizontalListDelegate respondsToSelector:@selector(MMHorizontalListView:didSelectCellAtIndex:)]) {
            [_horizontalListDelegate MMHorizontalListView:self didSelectCellAtIndex:index];
        }
    }
    if (_autoScrollPosition != MMHorizontalListViewPositionNone) {
        [self scrollToIndex:index animated:animated nearestPosition:_autoScrollPosition];
    }
    
    [_mainLock unlock];
}

- (void)_selectCellExcludeIndex:(NSInteger)index animated:(BOOL)animated responseToDelegate:(BOOL)responseToDelegate{
    
    NSArray<NSString *> *selectedIndexFrameStrings = [_selectedIndexes copy];
    for (NSString *selectedIndexFrameString in selectedIndexFrameStrings) {
        NSInteger selectedIndex = [_cellFrames indexOfObject:selectedIndexFrameString];
        if (selectedIndex != index) {
            [self _selectCellAtIndex:selectedIndex animated:animated responseToDelegate:responseToDelegate];
        }
    }
}
...
- (void)deselectCellAtIndex:(NSInteger)index animated:(BOOL)animated {
    if (_allowsMultipleSelection) {
        [self _deselectCellAtIndex:index animated:animated responseToDelegate:NO];
    }
}

- (void)_deselectCellAtIndex:(NSInteger)index animated:(BOOL)animated responseToDelegate:(BOOL)responseToDelegate{
    ...
    if (cell && [cell selected]) {
        ...
        if (responseToDelegate && [_horizontalListDelegate respondsToSelector:@selector(MMHorizontalListView:didDeselectCellAtIndex:)]) {
...
- (void)_deselectCellExcludeIndex:(NSInteger)index animated:(BOOL)animated responseToDelegate:(BOOL)responseToDelegate{
    
    NSArray<NSString *> *selectedIndexFrameStrings = [_selectedIndexes copy];
    for (NSString *selectedIndexFrameString in selectedIndexFrameStrings) {
        NSInteger selectedIndex = [_cellFrames indexOfObject:selectedIndexFrameString];
        if (selectedIndex != index) {
            [self _deselectCellAtIndex:selectedIndex animated:animated responseToDelegate:responseToDelegate];
        }
    }
}
...
- (void)highlightCellAtIndex:(NSInteger)index progress:(CGFloat)progress animated:(BOOL)animated {
    ...
    MMHorizontalListViewCell *cell = [_visibleCells objectForKey:frameString];
    if (cell) {
        [cell setHighlighted:YES progress:progress animated:animated];
    }
...
- (void)unhighlightCellAtIndex:(NSInteger)index progress:(CGFloat)progress animated:(BOOL)animated {
    ...
    MMHorizontalListViewCell *cell = [_visibleCells objectForKey:frameString];
    if (cell) {
        [cell setHighlighted:NO progress:progress animated:animated];
    }
...
- (void)setAllowsSelection:(BOOL)allowsSelection{
    if (_allowsSelection != allowsSelection) {
        ...
        if (!allowsSelection) {
            [self _deselectCellExcludeIndex:NSNotFound animated:YES responseToDelegate:YES];
        }
    }
}
...
- (void)setAllowsMultipleSelection:(BOOL)allowsMultipleSelection{
    if (_allowsMultipleSelection != allowsMultipleSelection) {
        ...
        if (!allowsMultipleSelection) {
            NSInteger defaultIndex = [_cellFrames indexOfObject:[_selectedIndexes firstObject]];
            [self _deselectCellExcludeIndex:defaultIndex animated:YES responseToDelegate:YES];
        }
    }
}
...
- (void)cellTap:(id)sender {
    ...
    if (_allowsMultipleSelection) {
        if (select) {
            [self _selectCellAtIndex:[cell index] animated:YES responseToDelegate:YES];
        } else {
            [self _deselectCellAtIndex:[cell index] animated:NO responseToDelegate:YES];
        }
    } else if (select && _allowsSelection){
        [self _deselectCellExcludeIndex:[cell index] animated:NO responseToDelegate:YES];
        [self _selectCellAtIndex:[cell index] animated:YES responseToDelegate:YES];
    }
}
...
- (void)updateIndex:(NSUInteger)index progress:(CGFloat)progress;{
    if (fabs(progress) <= 1) {
        [self unhighlightCellAtIndex:index progress:1 - fabs(progress) animated:YES];
        NSInteger highlightIndex = index + pow(-1, progress < 0);
        if (highlightIndex < [_cellFrames count]) {
            [self highlightCellAtIndex:highlightIndex progress:fabs(progress) animated:YES];
        }
    }
}
